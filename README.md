...
| 項目 | 費用 |
|---|---|
| Next.js ホスティング（Vercel Pro） | 約3,000円/月（商用利用のためProプラン必須。Hobbyは個人・非商用限定） |
| データベース（Supabase Free） | 0円/月（日々の問い合わせ対応で継続的にアクセスが発生するため、無料プランを想定。ただし1週間以上完全にアクセスがないとプロジェクトが自動停止する仕様あり） |
| Claude API（想定: 月300〜450件） | 約2,000〜3,000円/月 |
| 合計（上記3項目） | 約5,000〜6,000円/月 |

※上記に加えて、LINE公式アカウントの利用料金が別途発生する場合があります。友だち約300人への一斉配信は、配信のたびに「送信人数分」のメッセージ通数としてカウントされます。配信頻度が少なければ無料〜ライトプラン（月5,000円・5,000通まで）の範囲に収まる想定ですが、頻度が増える場合はスタンダードプラン（月15,000円〜）への移行が必要になることがあります。実際のプラン・料金は[LINEヤフー for Business](https://www.lycbiz.com/jp/service/line-official-account/plan/)の最新情報をご確認ください。
---
